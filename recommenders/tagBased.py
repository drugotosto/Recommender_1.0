...
class TagBased(ItemBased):
    ...
    def builtModel(self,spEnv,directory):
        ...
        if not os.path.exists(dirPathInput+"user_simsOrd(weightSim="+str(weightSim)+")/"):
            ...
            user_simsOrd=self.computeSimilarityTag(spEnv,tag_userVal_pairs,dictUser_meanRatesTags.value).map(lambda p: TagBased.filterSimilarities(p[0],p[1])).filter(lambda p: p!=None).map(lambda p: TagBased.nearestTagsNeighbors(p[0],p[1],nNeigh))
    ...
    def recommendationsUserBasedTag(user_id,users_with_sim,userHistoryRates,user_meanRates):
        ...
        """
            Rilascio una lista di suggerimenti composta da tutti gli items votati dagli utenti che risultano essere più simili
            all'active da un punto di vista degli interessi.
        """
    ...
    @staticmethod
    def nearestTagsNeighbors(user_id,users_and_sims,n):
        """
